...
from cassandra_fdw import logger, types_mapper
from cassandra_fdw.logger import WARNING
...
def import_schema(schema, srv_options, options, restriction_type, restricts):
  if ISDEBUG:
    logger.log(
      u"import schema {0} requiested with options {1}; restriction type: {2}; restrictions: {3}".format(
        schema, options, restriction_type, restricts))
  ...
  for c_table in cassandra_tables:
    ...
    if pg_table_name in mapping_dict:
      if ISDEBUG:
        logger.log(
          "Cassandra table name '{0}' maps to PostgreSQL table name '{1}'".format(pg_table_name,
                                                                                  mapping_dict[
                                                                                    pg_table_name]))
    ...
    for c_column_name in c_table.columns:
      ...
      if ISDEBUG:
        logger.log("Adding column {0} with PostgreSQL type {2} (CQL type {1})".format(c_column_name,
                                                                                      cql_type,
                                                                                      pg_type))
